...
- [窗口与终端](#窗口与终端)
- [文件、缓冲区与搜索](#文件缓冲区与搜索)
- [Copilot / OpenCode](#copilot--opencode)
- [Git 差分（gitsigns）](#git-差分gitsigns)
- [LSP 与诊断](#lsp-与诊断)
- [编辑](#编辑)
- [常见问题与排查 (Troubleshooting)](#常见问题与排查-troubleshooting)
...
### 鼠标拖拽选择文字时出现乱码 / 转义序列字符（如 `^[[<0;35;12M`）

- **原因**：Neovim 配置中启用了 `set mouse=a`（全模式鼠标捕获），导致终端模拟器接收到的是 Neovim 的鼠标事件协议坐标流，而不是终端原生的文本选择行为。在 Copilot / OpenCode / ToggleTerm 等聊天或交互式终端窗口中拖拽鼠标时尤为常见。
- **解决方法**：
  - **按住 `Shift` 键**同时用鼠标拖拽选择文字。这会绕过 Neovim 的鼠标捕获，恢复终端原生的选择与复制功能。
  - 或者在不需要鼠标时，通过 `:set mouse=` 临时关闭鼠标支持。
